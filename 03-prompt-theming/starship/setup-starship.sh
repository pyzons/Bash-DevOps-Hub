...
read -t 10 -p "Would you like to reload your bash profile to apply Starship? (Y/n): " reload_bash
if [[ ! "$reload_bash" =~ ^[Nn]$ ]]; then
    print_color $YELLOW "🔄 Reloading bash configuration..."
    source ~/.bashrc
    print_color $GREEN "✅ Configuration reloaded!"
else
    print_color $GREEN "✅ Setup complete! Restart your terminal or run 'source ~/.bashrc' to see the new prompt."
fi
